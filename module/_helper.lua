...
function extract_name_count( text )
	...
	if #name % 2 ~= 0 and name ~= 'Ｍ＆Ｍ\'ｓ' then message.warning( 'extract_name_count: extracting name from "' .. text .. '" results in: ' .. name ) end
	return name, count
end
...
function cn_timelen_to_sec( text )
	...
	if string.find( text, '负' ) then return 0 end
